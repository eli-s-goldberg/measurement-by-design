...
export class DistP {
  ...
  initialize() {
    ...
    if (this.distfunc === "fitted") {
      if (!this.samples) {
        throw new Error(
          "Samples must be provided when using 'fitted' distribution"
        );
      }
...
export class MatrixSolver {
  constructor({
    currentState = math.matrix([[3000], [6000], [2200], [1800], [200]]),
    transferMatrix = math.matrix([
      [0.75, 0.1, 0.1, 0.0, 0.0],
      [0.15, 0.85, 0.05, 0.0, 0.05],
      [0.05, 0.0, 0.8, 0.05, 0.05],
      [0.05, 0.1, 0.0, 0.95, 0.1],
      [0.0, 0.05, 0.05, 0.0, 0.85],
    ]),
    costMatrix = math.matrix([[2032], [105], [193], [90], [5]]),
  } = {}) {
  ...
  generateLatexEquation() {
    ...
    if (
      !Array.isArray(this.currentState.toArray()) ||
      !this.currentState.toArray().every(Array.isArray)
    ) {
    ...
    if (
      !Array.isArray(this.transferMatrix.toArray()) ||
      !this.transferMatrix.toArray().every(Array.isArray)
    ) {
